...
结论：初审时发现的质量门和重复维护任务已在当天修复；生产可运行，但仍保留 SearXNG、主编排规模、管理端边界和记忆生命周期等长期工程债。
...
- 表情包测试接受库初始化创建目录的既有契约。
- 群通用风格规则增加保守近义合并：触发场景和表达方式都必须各自共享明确语义标签；个人风格仍只使用精确 fingerprint，避免成员间串味。
- 新增“近义但表达不同，不得合并”回归测试。
- 修复后全量测试：485 通过。
...
**修复结果：** 已保留 `/etc/cron.d/qq-social-agent-hygiene` 作为唯一标准任务，并删除用户 crontab 中的重复行。后续观察每日日志确认仅运行一次。
