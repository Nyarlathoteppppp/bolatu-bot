...
- 轻量混合 RAG 将群聊原文、摘要、结构化记忆和画像统一重排；索引由后台增量同步，不阻塞回复热路径。已读取的群文件/网页可进入按群隔离的知识库，并支持人工检索反馈、回归评测和 Trace 命中详情。
- 群聊决策拆成确定性工具路由与轻量 Timing Gate；点名/明确工具直接继续，只有确认生成文字后才组装画像、RAG、风格和审批反馈。
