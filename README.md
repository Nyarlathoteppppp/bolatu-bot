...
QQ 客户端
  ...
  -> 前置筛选：去重、无固定延时的群内队列、工作强度、禁言和低价值消息拦截
...
#### 2. 前置筛选与群内队列：把无意义调用挡在 LLM 之外

每条群消息先经过确定性后端逻辑：持久去重、群内串行队列、工作强度概率、用户单独限频、禁言状态、低价值短句、图片/富媒体可见性和连续回复保护。普通消息没有固定缓冲等待，由 Timing Gate 判断是否插话。
工作强度默认 100%，手动调整后持续生效，直到再次调整。管理台首页的“群聊决策链与耗时”按消息展示关卡结果，并可打开对应 Trace 查看完整事件。
