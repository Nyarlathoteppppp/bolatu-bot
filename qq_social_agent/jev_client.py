...
class JevClient:
        ...
        questions = {
            "should_reply": {
                ...
                "instructions": (
                    ...
                    f"如果是被直接艾特、回复她、有清晰接梗吐槽点、被提问或被关心，应为 true；"
    ...
    async def select_speaking_action(
        ...
        *,
        ...
        if choice not in {'none': '不确定就不要加花活，维持普通接话/答题，不选其他动作', 'reply': '自然顺手接一句，没有更合适的细动作', 'answer': '先认真回答实际问题或给出判断，不要用玩梗代替答案', 'agree': '对方说到点子上，先认可再补一句自己的', 'care': '对方明显真难受、高压、撑不住，先接住再给可行的一句', 'tease': '普通吐槽、反差、轻微反击；对外人也能用的损，不要太亲昵', 'ask_back': '已经能短答或表态，再只追问一个真正有用或有趣的问题', 'at_someone': '必须把话甩给某个在场的人，并给他回复空间', 'observe': '轻轻冒泡，不是正式回答', 'echo_mood': '只接氛围（惊/无奈/高兴），不处理具体问题', 'shift_topic': '当前话题将死或没意思，轻轻转到更好聊的方向', 'self_comment': '被评价你自己，或需要自评/自嘲', 'relationship_reply': '必须用上和这个人的旧梗、称呼或长期关系', 'clarify': '还没听懂对象/问题，先对齐再答，不要装懂', 'warm_tease': '熟人之间更短更亲的损，撒娇式互损；生人不要', 'deflate': '有人膨胀、鸡汤、装逼，冷短扎一下就走，不讲笑话', 'take_side': '两造对比或吵架，明确站一边，不要和稀泥', 'share_self': '带一句自己的课/代码/申校/夜猫子日常来推进聊天，不是答题', 'comfort_joke': '倒霉、社死、小崩溃，用玩笑接住；真撑不住才用 care', 'mirror_style': '学当前这个人的句长、脏口密度、标点，内容仍是风雪的，禁止复读原句', 'amp_bit': '把群友刚抛的包袱/设定加一档当共谋，不复读同一句', 'deadpan_echo': '极短冷接，像草/好死，几乎不解释', 'commit_bit': '群友已在离谱设定里，认真演完这一下就出戏，不把玩笑当事实', 'hyperbole': '烦躁/倒霉用夸张狠话，抽象成修辞，不点名真人去死', 'wrong_register': '对离谱事突然一本正经或学术腔，错位才好笑', 'protect': '有人被围/被踩时挡一句，不是安慰也不是站队抬杠'}:
            choice = "none"
