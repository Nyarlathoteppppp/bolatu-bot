...
class VisionOcrConfig:
    ...
    missing_key_label: str = "vision ocr"


SiliconFlowOcrConfig = VisionOcrConfig
...
class VisionOcrClient:
    ...
    @classmethod
    def from_deepseek_config(cls, raw: object) -> "VisionOcrClient":
        cfg = raw if isinstance(raw, dict) else {}
        prompt = str(cfg.get("deepseek_vision_prompt") or cfg.get("siliconflow_prompt") or DEFAULT_VISION_PROMPT)
        return cls(
            VisionOcrConfig(
                enabled=bool(cfg.get("deepseek_vision_enabled", True)),
                base_url=str(cfg.get("deepseek_vision_base_url", "https://api.deepseek.com")),
                api_key_env=str(cfg.get("deepseek_vision_api_key_env", "DEEPSEEK_API_KEY")),
                model=str(cfg.get("deepseek_vision_model", "deepseek-flash")),
                timeout_seconds=float(cfg.get("deepseek_vision_timeout_seconds", 20.0)),
                max_tokens=int(cfg.get("deepseek_vision_max_tokens", 512)),
                detail=str(cfg.get("deepseek_vision_detail", "high")),
                max_file_bytes=int(cfg.get("deepseek_vision_max_file_bytes", 5 * 1024 * 1024)),
                prompt=prompt,
                missing_key_label="deepseek vision",
            )
        )

    @classmethod
    def from_siliconflow_config(cls, raw: object) -> "VisionOcrClient":
        cfg = raw if isinstance(raw, dict) else {}
        return cls(
            VisionOcrConfig(
                ...
                prompt=str(cfg.get("siliconflow_prompt", DEFAULT_VISION_PROMPT)),
                missing_key_label="siliconflow ocr",
            )
        )
    ...
    def _get_client(self) -> Any | None:
        ...
        if not api_key:
            if not self._warned_missing_key:
                logger.warning(
                    "qq_social_agent "
                    f"{self.config.missing_key_label} key missing: "
...
class SiliconFlowOcrClient(VisionOcrClient):
    @classmethod
    def from_config(cls, raw: object) -> "SiliconFlowOcrClient":
        client = cls.from_siliconflow_config(raw)
